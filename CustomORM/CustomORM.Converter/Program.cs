...
internal static class Program
{
    static void Main(string[] args)
    {
        ...
        foreach (var dv2HubFile in dv2FilesHubs)
        {
            ...
            // Find satellites for current hub
            var dv2FilesSatellites = Directory.GetFiles(folderSource, $"S{hubName[1..]}*", SearchOption.TopDirectoryOnly);

            // Get class content
            var contentEntityClass = BuildContentRelationalEntity(dv2HubFile, dv2ViewFile, dv2FilesSatellites, hubName[1..], namespaceTarget);

            // check class
            using (var peStream = new MemoryStream())
            {
                var result = CodeCompiler.CompileAssembly(contentEntityClass).Emit(peStream);
                ...
                else
                {
                    ...
                    // save file
                    SaveFile(folderTarget, $"{hubName[1..]}.cs", contentEntityClass);
                    // info
    ...
    static string BuildContentRelationalEntity(string hubPath, string viewPath, string[] satellitesPath, string entityName, string namespaceTarget)
    {
    ...
    static void SaveFile(string folderName, string fileName, string content)
    {
